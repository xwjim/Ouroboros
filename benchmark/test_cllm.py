...
class SyldModel:
    ...
    def run(self, input_ids, temperature = 1.0):
        # self.ngram_cache = CacheEngine(self.lookahead_level, self.guess_set_size)
        ...
        beg_time = time.time()
        output = self.test_func(input_ids, self.draft_model, self.target_model, self.ngram_cache, self.max_len, self.gamma, self.window_size, self.guess_set_size, self.lookahead_level, eos_token_id=self.tokenizer.eos_token_id)
        end_time = time.time()
...
def consistency_generate(
    ...
    itr = 0
    generation =  model.run(inputs['input_ids']) 
    return generation
    # while True:
    #     if itr == 0:
...
def get_raw_inputs(dev_set):
    ...
    if dev_set in ['all', 'gsm8k', 'math', 'mathgpt', 'gsm8k_robust']:
        with open(f'test.jsonl') as f:
            for cnt, line in enumerate(jsonlines.Reader(f)):
                if cnt > 50:
                    break
                data.append(line)
        if dev_set != 'all':
...
if __name__ == '__main__':
    ...
    if args.eval_only == False:
        ...
        #     device_map='cuda',
        # )
        small_model = CausalLM.from_pretrained("cllm/consistency-llm-7b-math", torch_dtype=torch.float16, device_map='auto')
        target_model = LlamaForCausalLM.from_pretrained("cllm/consistency-llm-7b-math", torch_dtype=torch.float16, device_map='auto')
        torch.cuda.empty_cache()

        tokenizer = AutoTokenizer.from_pretrained("cllm/consistency-llm-7b-math")
        model = SyldModel(draft_model=small_model, target_model=target_model, tokenizer=tokenizer, test_func=ouroboros, max_len=2048, gamma=16, window_size=16, guess_set_size=16, lookahead_level=16, model_type="llama")
        ...
        for processed_prompt in tqdm(processed_prompts):
            ...
            if args.use_consistency_decoding:
                output_ids = consistency_generate(
                    model,
                    tokenizer,
                    tokenizer([processed_prompt], return_tensors="pt").to("cuda"),
            ...
            if model.target_model.config.is_encoder_decoder:

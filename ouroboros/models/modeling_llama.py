    ...
    def jacobi_sample_multilevel(
        ...
        **model_kwargs,
        ...
        if continue_flag:
            ...
            past_logits = continue_ctx['past_logits']
        ...
        while True:
            ...
            past_logits = torch.cat((past_logits,outputs.logits),dim=1) if past_logits is not None else outputs.logits
            for hh in range(max_hit):
                past_logits[:,outputs.kvcache_len + hh,:] = past_logits[:,outputs.step_len-len(guess_tokens)+max_hit_idx * GUESS_SIZE + hh,:]
            past_logits = past_logits[:,:outputs.kvcache_len + max_hit,:]
            assert past_logits.shape[1] == outputs.past_key_values[0][0].shape[2]
    ...
    def jacobi_greedy_search_multilevel(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            past_logits = torch.cat((past_logits,outputs.logits),dim=1) if past_logits is not None else outputs.logits
            for hh in range(max_hit):
                past_logits[:,outputs.kvcache_len + hh,:] = past_logits[:,outputs.step_len-len(guess_tokens)+hit_point * GUESS_SIZE + hh,:]
            past_logits = past_logits[:,:outputs.kvcache_len + max_hit,:]
            assert past_logits.shape[1] == outputs.past_key_values[0][0].shape[2]
